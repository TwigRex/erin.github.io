...
const ribbonMove = get('#Ribbon_move');
...
const mouseOver = () => {
	ribbonMove.style.transform = 'rotate(15deg)';
	if(!ribbonMove.classList.value.includes('swat')) {
		ribbonMove.classList.add('swat');
...
const mouseOut = () => {
	if(ribbonMove.classList.value.includes('swat')) {
		ribbonMove.classList.remove('swat');
...
const mouseNear = (coords, x, y) => {
	...
	ribbonMove.style.transform = `rotate(${30 / (zeroX + zeroY) * (x + y)}deg)`;
}
