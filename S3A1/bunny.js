...
const mouseOver = () => {
	earMove.style.transform = 'rotate(-20deg)';
	if(!earMove.classList.value.includes('swat')) {
		earMove.classList.add('swat');
...
const mouseNear = (coords, x, y) => {
	...
	earMove.style.transform = `rotate(-${60 / (zeroX + zeroY) * (x + y)}deg)`;
}
...
tracker.addEventListener('mousemove', e => {
	...
	if(overTheButton(btnCoords, x, y)) {
		...
		if (nearTheButton(btnCoords, x, y)){
			...
			earMove.style.transform = 'rotate(10deg)';
		}
	}
